...
### Fixed
- **Offline Device Detail Polling**: Skips detail refreshes for devices already marked offline by the VeSync device list, reducing avoidable API calls and noisy failures while preserving cached state until the device returns online.

## [1.4.9] - 2026-03-30

### Fixed
- **💧 Dual 200S Humidifier Write Method**: Scoped `bypassV2` `PUT` writes for `setMode()` and `setHumidity()` to Dual 200S / LUH-D301S variants only, because those models can silently ignore `POST` even when VeSync reports success. This fixes confirmed humidifier writes without changing behavior for unaffected models.

## [1.4.8] - 2026-03-27

### Changed
- Version sync with homebridge-tsvesync for the Dual 200S humidifier stale-refresh write fix (no functional library changes).
...
### Fixed
- **🔑 Token Expiration Recovery (-11001022)**: Treats `-11001022` ("the token has expired") as a token error and performs a one-time re-login + retry, including cases where the API returns HTTP 200 with an error code in the response body.

## [1.4.5] - 2026-01-19

### Changed
- **🧩 Node 24 Compatibility**: Relaxed `engines.node` to allow installation on Node 24.x (no runtime changes).
